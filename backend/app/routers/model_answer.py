...
def _format_chunks(chunks: list[dict]) -> str:
    """search_resume Í≤∞Í≥ºÎ•??ÑÎ°¨?ÑÌä∏???çÏä§?∏Î°ú ÏßÅÎ†¨??"""
    if not chunks:
        return "(?¥Î†•??Í¥Ä??Ï≤?Å¨ ?ÜÏùå)"
    lines = []
    for c in chunks:
...
async def generate_model_answer(
    ...
    if not body.resumeId:
        raise HTTPException(400, {"error": "¿Ã∑¬º≠ ID∞° « ø‰«’¥œ¥Ÿ"})
    ...
    try:
        ...
        raise HTTPException(500, {"error": "AI ?ùÏÑ±???§Ìå®?àÏäµ?àÎã§"})

    if not questions:
        raise HTTPException(500, {"error": "AI ?ùÏÑ±???§Ìå®?àÏäµ?àÎã§"})

    # Step 2: Î™®Î≤î?µÏïà Í∞úÎ≥Ñ Î≥ëÎ†¨ ?ùÏÑ±
    ...
    async def _gen_answer(q: dict) -> dict | None:
        try:
            if use_rag:
                ...
                resume_context = parsed_resume or "(?¥Î†•???ïÎ≥¥ ?ÜÏùå)"
